...
interface SelectedItemsProps {
  selectedItems: SubItemWithParent[];
  handleToggle: (item: SubItemWithParent) => void;
}

export const SelectedItems = ({ selectedItems, handleToggle }: SelectedItemsProps) => {
  return (
    <div className="selected-box" role="region" aria-label="선택한 분야 목록">
      {selectedItems.length > 0 ? (
        <div className="data flex flex-wrap gap-5" role="list">
          {selectedItems.map(({ parent, name, code }) => {
            const key = `${parent}-${name}`;
            const label = `${parent} > ${name}`;

            return (
              <span key={key} role="listitem" className="items-center bg-[#E6F4F1] text-gray-800 rounded-full px-3 py-1 text-sm">
                {label}
                <button onClick={() => handleToggle({ parent, name, code })} className="ml-2" aria-label={`${label} 필터 삭제`}>
                  <svg width={12} height={11} viewBox="0 0 12 11" fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none">
                    <path
        ...
        <Typography className="no-data" variant="body" size="14" color="grayscale" role="status" aria-live="polite">
          분야를 선택해 주세요
        </Typography>
      )}
    </div>
