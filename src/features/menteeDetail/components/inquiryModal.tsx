...
export const InquiryModal = ({ id, onClose, onDone }: InquiryModalProps) => {
  ...
  const handleSubmit = () => {
    ...
    const confirmed = window.confirm("문의 내용을 멘토에게 보내시겠습니까?\n문의 내용은 멘토링챗 > 임시채팅 내에서 확인할 수 있습니다.");
  ...
  return (
    <>
      <Modal id={id} title="문의하기" closeModal={() => onClose()}>
        <div className="inquiry-modal__body">
          <div className="inquiry-modal__text-wrapper">
            <textarea className="inquiry-modal__textarea" placeholder="문의하실 내용을 입력해 주세요" maxLength={maxLength} value={content} onChange={handleChange} />
          </div>
          ...
          <p className="inquiry-modal__note">
            문의는 한 번만 보낼 수 있고, 보낸 후에는 내용을 수정이 어렵습니다.
            <br />
