    ...
    <>
      <div className="info-box">
        ...
        <div className="info-row">
          ...
          <div className="info-value">
            <ToggleSwitch checked={emailAgree} onChange={(val) => setEmailAgree(val)} />
          </div>
        </div>
        <div className="info-row">
          <div className="info-label">SMS 수신 동의</div>
          <div className="info-value">
            <ToggleSwitch checked={smsAgree} onChange={(val) => setSmsAgree(val)} />
          </div>
        </div>
      </div>
      <div className="info-withdraw">
        <button>
          회원 탈퇴 <Icon src={"chevron_right"} alt="탈퇴하기" />
        </button>
      </div>

      {/* 닉네임 변경 모달 */}
      {isNicknameModalOpen && <NicknameEditModal id="nickname-edit" closeModal={() => setIsNicknameModalOpen(false)} />}

      {/* 비밀번호 변경 모달 */}
      {isPasswordModalOpen && <PasswordEditModal id="nickname-edit" closeModal={() => setIsPasswordModalOpen(false)} />}
